"""This class is used to store the code for the TurboFan window of the PCA-EPA software."""
...
# from ..MAIN_BACKEND import plots_file, initialisingComponents_file
...
from EPA_window import EPA_window_class
...
class TF_window_class:
    ...
    def epa_test_button_command(self):
        if self.var_single_dual.get()=="dual":
            messagebox.showinfo("Info","EPA works best when pi_cL is close to pi_f. Please adjust the values accordingly before proceeding.")
    ...
    def reset_parameters_widgets_frame(self,event=None):        #event=None is added bcz when we call this function from bind it will pass an event argument
        ...
        #if the user has selected the sep_flow switch button, then afterburner checkbutton should be automcatically unchecked
        if self.var_sep_mixed.get()=="sep":
            self.var_AB.set("N")
    ...
    #atmosphere conditions widgets frame  inside parameters_widgets_frame
        ...
        #alp_entry widget inside atmsophere_widgets_frame
        if self.single_multi_selected_button=="multi" and ((self.onetwo_indp_var.get()=="one" and "alp" in [self.indep_var_entry_combobox.get()]) or (self.onetwo_indp_var.get()=="two" and "alp" in [self.indep_var1_entry_combobox.get(),self.indep_var2_entry_combobox.get()])) :
            alp_label= ttk.Label(atmosphere_widgets_frame,text= "Bypass Ratio \u03b1")
            alp_label.grid(row=row_num,column=0,padx= 5,pady=(0,5),sticky="ew")

            self.alp_iter_frame= ttk.Frame(atmosphere_widgets_frame)
            alp_iter_frame= self.alp_iter_frame
            alp_iter_frame.grid(row=row_num,column=1,padx= 0,pady=0,sticky="ew")
            self.min_alp_entry= ttk.Entry(alp_iter_frame)
            min_alp_entry= self.min_alp_entry
            min_alp_entry.grid(row=0,column=0,padx= 0,pady=(0,5),sticky="w")
            if self.var_sep_mixed.get()=="mixed":
                min_alp_entry.insert(0,"NA")
            elif self.var_bool:
                min_alp_entry.insert(0,min_alp_default)
            elif self.inpt_outpt_selected_button=="output"and hasattr(self,"parameters_dict"):
                min_alp_entry.insert(0,min_alp)
            else:
                min_alp_entry.insert(0,"Min")
            min_alp_entry.bind("<FocusIn>",lambda args: min_alp_entry.delete('0','end'))
            min_alp_entry.config(width=new_width)
            self.steps_alp_entry= ttk.Spinbox(alp_iter_frame,from_= 0,to= 1000)
            steps_alp_entry= self.steps_alp_entry
            steps_alp_entry.grid(row=0,column=1,padx= 0,pady=(0,5),sticky="ew")
            if self.var_sep_mixed.get()=="mixed":
                steps_alp_entry.insert(0,"NA")
            elif self.var_bool:
                steps_alp_entry.insert(0,steps_alp_default)
            elif self.inpt_outpt_selected_button=="output"and hasattr(self,"parameters_dict"):
                steps_alp_entry.insert(0,steps_alp)
            else:
                steps_alp_entry.insert(0,"Steps")
            steps_alp_entry.bind("<FocusIn>",lambda args: steps_alp_entry.delete('0','end'))
            steps_alp_entry.config(width=new_width)
            self.max_alp_entry= ttk.Entry(alp_iter_frame)
            max_alp_entry= self.max_alp_entry
            max_alp_entry.grid(row=0,column=2,padx= 0,pady=(0,5),sticky="e")
            if self.var_sep_mixed.get()=="mixed":
                max_alp_entry.insert(0,"NA")
            elif self.var_bool:
                max_alp_entry.insert(0,max_alp_default)
            elif self.inpt_outpt_selected_button=="output"and hasattr(self,"parameters_dict"):
                max_alp_entry.insert(0,max_alp)
            else:
                max_alp_entry.insert(0,"Max")
            max_alp_entry.bind("<FocusIn>",lambda args: max_alp_entry.delete('0','end'))
            max_alp_entry.config(width=new_width)
            row_num+=1  

        else :    #if single point calculation is selected or if multi point then user has not selected Tt4 as independent variable
            alp_label= ttk.Label(atmosphere_widgets_frame,text= "Bypass Ratio \u03b1")
            alp_label.grid(row=row_num,column=0,padx= 5,pady=(0,5),sticky="nsew")
            self.alp_entry= ttk.Entry(atmosphere_widgets_frame)
            alp_entry= self.alp_entry
            if self.var_sep_mixed.get()=="mixed":
                alp_entry.insert(0,"NA")
            elif self.var_bool:
                alp_entry.insert(0,alp_default)
            elif self.inpt_outpt_selected_button=="output"and (hasattr(self,"engine") or hasattr(self,"parameters_dict")):
                alp_entry.insert(0,alp)
            else:
                alp_entry.insert(0,"Bypass Ratio")
            alp_entry.bind("<FocusIn>",lambda args: alp_entry.delete('0','end'))
            alp_entry.grid(row=row_num, column=1, padx= 5,pady=(0,5),sticky="nsew")
            row_num+=1
    ...
    #mixer_properties_widgets_frame 
        if self.var_sep_mixed.get()=="mixed":
            mixer_properties_widgets_frame= ttk.LabelFrame(parameters_widgets_frame,text="Mixer Properties")
            mixer_properties_widgets_frame.grid(column=0,row=3,padx= 5, pady= 5,sticky=tk.N+tk.W+tk.E)
            row_num= 0

            if self.var_ideal_real.get()=="real":
                #pi_m_max_entry widget inside pressures_ratios_widgets_frame
                pi_m_max_label= ttk.Label(mixer_properties_widgets_frame,text= syb("pi_m_max"))
                pi_m_max_label.grid(row=row_num,column=0,padx= 5,pady=(0,5),sticky="ew")
                self.pi_m_max_entry= ttk.Entry(mixer_properties_widgets_frame)
                pi_m_max_entry= self.pi_m_max_entry
                if self.var_bool:
                    pi_m_max_entry.insert(0,pi_m_max_default)
                elif self.inpt_outpt_selected_button=="output"and (hasattr(self,"engine") or hasattr(self,"parameters_dict")):
                    pi_m_max_entry.insert(0,pi_m_max)
                else:
                    pi_m_max_entry.insert(0,"pi_m_max")
                pi_m_max_entry.bind("<FocusIn>",lambda args: pi_m_max_entry.delete('0','end'))
                pi_m_max_entry.grid(row=row_num,column=1,padx= 5,pady=(0,5),sticky="ew")
                row_num+=1
                        
            #M6_entry widget inside mixer_properties_widgets_frame
            M6_label= ttk.Label(mixer_properties_widgets_frame,text= syb("M6"))
            M6_label.grid(row=row_num,column=0,padx= 5,pady=(0,5),sticky="ew")
            self.M6_entry= ttk.Entry(mixer_properties_widgets_frame)
            M6_entry= self.M6_entry
            if self.var_bool:
                M6_entry.insert(0,M6_default)
            elif self.inpt_outpt_selected_button=="output"and (hasattr(self,"engine") or hasattr(self,"parameters_dict")):
                M6_entry.insert(0,M6)
            else:
                M6_entry.insert(0,"M6")
            M6_entry.bind("<FocusIn>",lambda args: M6_entry.delete('0','end'))
            M6_entry.grid(row=row_num,column=1,padx= 5,pady=(0,5),sticky="ew")
            row_num+=1
        ...
        #efficiencies widgets frame inside parameters_widgets_frame
            ...
            peta_f_label= ttk.Label(efficiencies_widgets_frame,text= syb("peta_f"))
            peta_f_label.grid(row=row_num,column=0,padx= 5,pady=(0,5),sticky="ew")
            self.peta_f_entry= ttk.Entry(efficiencies_widgets_frame)
            peta_f_entry= self.peta_f_entry
            if self.var_bool:
                peta_f_entry.insert(0,peta_f_default)
            elif self.inpt_outpt_selected_button=="output"and (hasattr(self,"engine") or hasattr(self,"parameters_dict")):
                peta_f_entry.insert(0,peta_f)
            else:
                peta_f_entry.insert(0,"Polytropic eff Fan")
            peta_f_entry.bind("<FocusIn>",lambda args: peta_f_entry.delete('0','end'))
            peta_f_entry.grid(row=row_num,column=1,padx= 5,pady=(0,5),sticky="ew")
            row_num+=1
    ...
    #Pressures ratios widgets frame inside parameters_widgets_frame
        ...
        #pi_f_entry widget inside pressures_ratios_widgets_frame
        pi_f_label= ttk.Label(pressures_ratios_widgets_frame,text= syb("pi_f"))
        pi_f_label.grid(row=row_num,column=0,padx= 5,pady=(0,5),sticky="ew")
        self.pi_f_entry= ttk.Entry(pressures_ratios_widgets_frame)
        pi_f_entry= self.pi_f_entry
        if self.var_bool:
            pi_f_entry.insert(0,pi_f_default)
        elif self.inpt_outpt_selected_button== "output" and (hasattr(self,"engine") or hasattr(self,"parameters_dict")):
            pi_f_entry.insert(0,pi_f)
        else:
            pi_f_entry.insert(0,"Fan PR")
        pi_f_entry.bind("<FocusIn>",lambda args: pi_f_entry.delete('0','end'))
        pi_f_entry.grid(row=row_num,column=1,padx= 5,pady=(0,5),sticky="ew")
        row_num+=1
        ...
        if self.var_ideal_real.get()=="real":
            ...
            if self.var_sep_mixed.get()=="sep":
                #pi_fn_entry widget inside pressures_ratios_widgets_frame
                pi_fn_label= ttk.Label(pressures_ratios_widgets_frame,text= syb("pi_fn"))
                pi_fn_label.grid(row=row_num,column=0,padx= 5,pady=(0,5),sticky="ew")
                self.pi_fn_entry= ttk.Entry(pressures_ratios_widgets_frame)
                pi_fn_entry= self.pi_fn_entry
                if self.var_bool:
                    pi_fn_entry.insert(0,pi_fn_default)
                elif self.inpt_outpt_selected_button=="output"and (hasattr(self,"engine") or hasattr(self,"parameters_dict")):
                    pi_fn_entry.insert(0,pi_fn)
                else:
                    pi_fn_entry.insert(0,"Fan Nozzle PR")
                pi_fn_entry.bind("<FocusIn>",lambda args: pi_fn_entry.delete('0','end'))
                pi_fn_entry.grid(row=row_num,column=1,padx= 5,pady=(0,5),sticky="ew")
                row_num+=1
        ...
        #nozzle_properties widgets frame inside parameters_widgets_frame
            ...
            P0_P9_label.grid(row=row_num,column=0,padx= 5,pady=(0,5),sticky="ew")
            ...
            P0_P9_entry.grid(row=row_num,column=1,padx= 5,pady=(0,5),sticky="ew") 
            row_num+=1

            if self.var_sep_mixed.get()=="sep":
                #P0/P19_entry widget inside nozzle_properties_widgets_frame
                P0_P19_label= ttk.Label(nozzle_properties_widgets_frame,text= "P0/P19")
                P0_P19_label.grid(row=row_num,column=0,padx= 5,pady=(0,5),sticky="ew")
                self.P0_P19_entry= ttk.Entry(nozzle_properties_widgets_frame)
                P0_P19_entry= self.P0_P19_entry
                if self.var_bool:
                    P0_P19_entry.insert(0,P0_P19_default)
                elif self.inpt_outpt_selected_button=="output"and (hasattr(self,"engine") or hasattr(self,"parameters_dict")):
                    P0_P19_entry.insert(0,P0_P19)
                else:
                    P0_P19_entry.insert(0,"P0/P19")
                P0_P19_entry.bind("<FocusIn>",lambda args: P0_P19_entry.delete('0','end'))
                P0_P19_entry.grid(row=row_num,column=1,padx= 5,pady=(0,5),sticky="ew") 
                row_num+=1
    ...
    def set_default_values(self):
        ...
        if self.single_multi_selected_button=="single":
            ...
            pi_c_default= 24   #bcz what if it is not selected as independent variable
            min_pi_c_default= 16
            steps_pi_c_default= 5
            max_pi_c_default= 24
        pi_cL_default= 3.8
        pi_cH_default= 6.3157
        pi_b_default= 0.96
        pi_m_max_default= 0.97
        pi_ab_default= 0.96
        pi_n_default= 0.98
        pi_fn_default= 0.98
        peta_f_default= 0.89
    ...
    def set_parameters(self):
        ...
        global min_alp,steps_alp,max_alp
        ...
        try:
            ...
            if self.single_multi_selected_button=="single":
                ...
                if self.var_sep_mixed.get()=="sep":
                    if (self.onetwo_indp_var.get()=="one" and "alp" in [self.indep_var_entry_combobox.get()]) or  self.onetwo_indp_var.get()=="two" and "alp" in [self.indep_var1_entry_combobox.get(),self.indep_var2_entry_combobox.get()]:      
                        min_alp= float(self.min_alp_entry.get())
                        steps_alp= float(self.steps_alp_entry.get())
                        max_alp= float(self.max_alp_entry.get())
                        alp= '-'
                    else:
                        alp= float(self.alp_entry.get())
                else:
            ...
            pi_f= float(self.pi_f_entry.get())
            ...
            if self.var_sep_mixed.get()=="mixed":
                M6= float(self.M6_entry.get())
                if self.var_ideal_real.get()=="real":
                    pi_m_max= float(self.pi_m_max_entry.get())
                else:
                    pi_m_max= "NA"
                sep_mixed_flow= "M"
            else:
            ...
            if self.var_ideal_real.get()=="real":
                ...
                peta_f= float(self.peta_f_entry.get())
            ...
            if self.var_ideal_real.get()=="real":
                ...
                if self.var_sep_mixed.get()=="sep":
                    pi_fn= float(self.pi_fn_entry.get())
                else:
                    pi_fn= 1
                if self.var_AB.get()=="Y":
            ...
            if self.var_ConDiv_Con.get()=="C":
                ...
                if self.var_sep_mixed.get()=="sep":
                    P0_P19= float(self.P0_P19_entry.get())
                else:
                    P0_P19= "NA"                   #its value will be automatically be equal to P0_P9
    ...
    def set_iteration_variables_frame(self):
        ...
        if self.onetwo_indp_var.get()=="one":
            ...
            self.indep_var_entry_combobox= ttk.Combobox(self.iteration_variables_frame,values=["Tt4","pi_c","alt","M0","alp"],state="readonly")
            ...
            if self.inpt_outpt_selected_button=="output" and hasattr(self,"parameters_dict") and hasattr(self,"indep_var_iterlst"):
                ...
                elif self.indep_var_iterlst[0]=="alp":
                    indep_var_entry_combobox.set("alp")
            else:
            ...
            self.indep_var1_entry_combobox= ttk.Combobox(self.iteration_variables_frame,values=["Tt4","pi_c","alt","M0","alp"],state="readonly")
            ...
            if self.inpt_outpt_selected_button=="output" and hasattr(self,"parameters_dict") and hasattr(self,"indep_var1_iterlst"):
                ...
                elif self.indep_var1_iterlst[0]=="alp":
                    indep_var1_entry_combobox.set("alp")              
            else:
            ...
            self.indep_var2_entry_combobox= ttk.Combobox(self.iteration_variables_frame,values=["Tt4","pi_c","alt","M0","alp"],state="readonly")
            ...
            if self.inpt_outpt_selected_button=="output" and hasattr(self,"parameters_dict") and hasattr(self,"indep_var2_iterlst"):
                ...
                elif self.indep_var2_iterlst[0]=="alp":
                    indep_var2_entry_combobox.set("alp")
            else:
    ...
    def inputs_button_command(self):
        """This function is used to create the input window for the turbofan engine."""
    ...
    #main_widgets_frame widget to contain the ideal/reality, single/dual, afterburner and CD/C switch buttons
        ...
        #sep_mixed_checkbutton widget
        if self.inpt_outpt_selected_button =="input":
            self.var_sep_mixed= tk.StringVar(value= "sep")      
        self.sep_mixed_switch= ttk.Checkbutton(main_widgets_frame,variable=self.var_sep_mixed,style="Switch",offvalue="sep",onvalue="mixed",text="Mixed Exhaust",command= self.reset_parameters_widgets_frame)
        sep_mixed_switch= self.sep_mixed_switch
        sep_mixed_switch.grid(row=0,column=3,padx= pad_x_val, pady= 5,sticky="ew")
        ...
        afterburner_checkbutton.grid(row=0,column=4,padx= pad_x_val, pady= 5,sticky="ew")

        #CD_C_switchbutton widget   #convergingDiverging/Converging
        if self.inpt_outpt_selected_button =="input":
            self.var_ConDiv_Con= tk.StringVar(value="CD")  #setting default value
        self.CD_C_switch= ttk.Checkbutton(main_widgets_frame,variable=self.var_ConDiv_Con,style="Switch",offvalue="CD",onvalue="C",text="Convergent nozzle",command= self.reset_parameters_widgets_frame) #style="Switch",text="CD/C"
        CD_C_switch= self.CD_C_switch
        CD_C_switch.grid(row=0,column=5,padx= pad_x_val, pady= 5,sticky="ew")
    ...
    def set_PCA_results_single_point_calculation(self):
        ...
        for i in lst0:
            ...
            if type(val2) != str:
                val2= round(val2,4)
            self.station_properties_tree_view.insert("",tk.END,values=(i[0],val1,val2))
    ...
    def set_outputs_frame_single_point_calculation(self):
        """This function is used to create the output window for the turbofan engine when single point calculation is selected."""
    ...
    #nomenclature_img label frame 
        ...
        if self.var_sep_mixed.get()=="sep":
            # img= Image.open("\\".join(os.path.abspath(__file__).split("\\")[0:-1]) +"\\images\\turbofan_nomenclature_sep_img.png")
            img= Image.open(resource_path("MAIN_GUI\\images\\turbofan_nomenclature_sep_img.png"))
        else:
            # img= Image.open("\\".join(os.path.abspath(__file__).split("\\")[0:-1]) +"\\images\\turbofan_nomenclature_mixed_img.png")
            img= Image.open(resource_path("MAIN_GUI\\images\\turbofan_nomenclature_mixed_img.png"))
    ...
    #EPA testing frame just below the station_properties_frame
        ...
        if self.var_sep_mixed.get()=="mixed" or self.var_single_dual.get()=="single":
            epa_test_button= ttk.Button(EPA_testing_frame,text="Test this engine",command= lambda: messagebox.showinfo("Error","This feature is not available for mixed-exhaust or single-spool turbofan engines"))
        else:
            epa_test_button= ttk.Button(EPA_testing_frame,text="Test this engine",command= self.epa_test_button_command) #    def epa_test_button_command(self):
    ...
    def set_outputs_frame_multi_point_calculation(self):
        """This function is used to create the output window for the turbofan engine when multi point calculation is selected."""
    ...
    def pca_results_vs_station_properties_checkbutton_command(self,event=None):  #bcz we are calling this function from the index_entry widget hence we need 
        ...
        #epa_test_button widget        
        if self.var_sep_mixed.get()=="mixed" or self.var_single_dual.get()=="single":
            epa_test_button= ttk.Button(self.wrapper_tree_frame,text="Test this engine",command= lambda: messagebox.showinfo("Error","This feature is not available for mixed-exhaust or single-spool turbofan engines"))
        else:
            epa_test_button= ttk.Button(self.wrapper_tree_frame,text="Test this engine",command= self.epa_test_button_command) #    def epa_test_button_command(self):
        epa_test_button.grid(row=1,column=0,columnspan=2,padx=(10,5),pady=(5,0))


    def outputs_button_command(self):
        """This function is used to create the output window for the turbofan engine."""
    ...
    def on_resize(self,event):
        ...
        frame_width = self.root_frame.winfo_reqwidth()
        new_x_position = max(0, (canvas_width - frame_width) // 2)
        
        # Move the root_frame to the center
        self.my_canvas.coords(self.root_frame_window, new_x_position, 0)
        # Update the scroll region to encompass the root_frame
        self.my_canvas.configure(scrollregion=self.my_canvas.bbox("all"))

        if self.selected_theme=="forest-dark":
            self.my_canvas.config(bg='#313131')    
        else:
            self.my_canvas.config(bg='#ffffff')

    def create_TF_window(self):
        self.root2= tk.Tk()
        root2= self.root2
        self.screen_width= 860
        root2.geometry(f"{self.screen_width}x{self.screen_width-60}")
        # Bind the resize event
        root2.bind('<Configure>', self.on_resize)
        root2.resizable(True,True)
        root2.title("TurboFan Engine")

        self.main_frame = ttk.Frame(root2)
        main_frame= self.main_frame
        main_frame.pack(fill="both", expand=True)

        self.my_canvas = tk.Canvas(main_frame)
        my_canvas= self.my_canvas
        ...
        self.root_frame = ttk.Frame(my_canvas)
        root_frame= self.root_frame
        self.root_frame_window = my_canvas.create_window((0,0), window=root_frame, anchor="nw")
